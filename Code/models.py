...
class Res_BERT(nn.Module):
    ...
    def forward(self, input_ids, visual_embeds_att, input_mask, labels=None):
...
class BertOnly(nn.Module):
    ...
    def forward(self, input_ids, visual_embeds_att, input_mask, added_attention_mask, objtag_input_ids,
                objtag_input_mask, labels=None):
        sequence_output, pooled_output = self.bert(input_ids=input_ids, token_type_ids=None, attention_mask=input_mask)
...
class CDKI(nn.Module):
    def __init__(self):
        super(CDKI, self).__init__()
        self.bert = BertModel.from_pretrained('bert-base-chinese')
        self.dropout = nn.Dropout(0.1)
        self.classifier = nn.Linear(768, 2)
        self.vismap2text = nn.Linear(2048, 768)
        ...
        self.objtag_bert = BertModel.from_pretrained('bert-base-chinese')
        self.text2objtag_attention = BertCrossEncoder()
        self.objtag_text_pooler = BertPooler()
    ...
    def forward(self, input_ids, visual_embeds_att, input_mask, added_attention_mask, objtag_input_ids,
        ...
        objtag_output, objtag_pooled_output = self.objtag_bert(input_ids=objtag_input_ids, token_type_ids=None,
                                                                  attention_mask=objtag_input_mask)
        ...
        text_objtag_cross_attn = self.text2image_attention(sequence_output, objtag_output, [])
        text_objtag_pooled_output = self.objtag_text_pooler(text_objtag_cross_attn)

        pooled_output = torch.cat([image_text_pooled_output, text_objtag_pooled_output], dim=-1)
...
class Res_BERT_Pair_2(nn.Module):
    # 1.把矩阵加载进来tensor
    # 2.把msdbert的attention拿进来
    # 3.查看sequence_output的维度，把矩阵换成和它维度相同，目标是能运行
    def __init__(self):
        super(Res_BERT_Pair_2, self).__init__()
        self.bert = BertModel.from_pretrained('bert-base-chinese')
        self.dropout = nn.Dropout(0.1)
        self.classifier = nn.Linear(768, 2)
        ...
        self.image_text_pooler = BertPooler()
        self.objtag_bert = BertModel.from_pretrained('bert-base-chinese')
        # 读取词向量文件，每行转化为一个长度为768的一维numpy数组
        with open('autodl-tmp/baseline/myarray.txt', 'r') as f:
            lines = f.readlines()
            word_vecs = [np.array(line.strip().split(), dtype=float) for line in lines]

        # 将一维numpy数组拼接成二维numpy数组
        word_vecs = np.stack(word_vecs)

        # 转换为tensor
        self.pair_tensor = torch.from_numpy(word_vecs)
        if torch.cuda.is_available():
            self.pair_tensor = self.pair_tensor.to('cuda:0')
        # print('self.pair_tensor.shape',self.pair_tensor.shape)
        # print('self.pair_tensor.dtypr',self.pair_tensor.dtype)
    ...
    def forward(self, input_ids, visual_embeds_att, input_mask, added_attention_mask, objtag_input_ids,
        ...
        # print('pooled_output.dtype',pooled_output.dtype)
        ...
        # b*49*768
        visual = self.vismap2text(vis_embed_map)
        # if self.pair_tensor.shape[0] != visual.shape[0]:
        #     self.pair_tensor = self.pair_tensor.repeat(visual.shape[0], 1, 1)
        
        self.pair_tensor = self.pair_tensor.unsqueeze(0).repeat(visual.shape[0], 1, 1)
        
        # print("self.pair_tensor.size111111111", self.pair_tensor.shape)
        # print('self.pair_tensor.dtyp',self.pair_tensor.dtype)
        self.pair_tensor = self.pair_tensor.to(dtype = torch.float)
        self.pair_tensor = self.Linear2(self.pair_tensor)
        image_text_cross_attn = self.text2image_attention(self.pair_tensor, visual, extended_img_mask)
        self.pair_tensor = self.pair_tensor[0].squeeze(0)
        # print("self.pair_tensor.size222222222", self.pair_tensor.shape)
        # b*75*12
        # C = self.tanh(torch.matmul(torch.matmul(sequence_output, self.W_b), objtag_output.transpose(1,2)))
        # # C: b*12
        # C, _ = torch.max(C, dim=1)
        # attn = F.softmax(C, dim=-1)
        # b*1*768
        # print("image_text_cross_attn===", image_text_cross_attn.shape)
        image_text_pooled_output = self.image_text_pooler(image_text_cross_attn)
        # print("image_text_pooled_output===", image_text_pooled_output.shape)

        # objtag_text_cross_attn = torch.matmul(attn.unsqueeze(1), objtag_output)
        pooled_output = pooled_output.to(dtype = torch.float)
        pooled_output = torch.cat([image_text_pooled_output, pooled_output.squeeze(1)], dim=-1)
        # b*1*768
        res = torch.cat([sequence_output, visual], dim=1).mean(1)
        pooled_output = self.dropout(res)
        logits = self.classifier(pooled_output)
